...
void main() {
  vec4 color_material = BlinnPhong(fragN, fragL, fragV);

  if (gl_FrontFacing) {

    outColor = mix(color_material, color, vec4(0.5f));
  } else {
    float i = (color_material.r + color_material.g + color_material.b) / 3.0;
